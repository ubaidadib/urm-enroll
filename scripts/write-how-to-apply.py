...
export function HowToApplyGermanUniversityPage() {
  ...
  return (
    <div className="min-h-screen bg-background-primary">
      ...
      {/* Hero */}
      <div className="pt-24 pb-10 px-6 bg-background-surface border-b border-border">
        ...
        <div className="flex flex-col lg:flex-row gap-10">
            ...
            <div className="sticky top-24 space-y-1">
              ...
              {TOC_IDS.map((id) => (
                <a
                  key={id}
                  href={`#${id}`}
                  className="flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-text-secondary hover:text-text-primary hover:bg-background-surface transition-colors"
                >
                  <span className="w-1.5 h-1.5 rounded-full bg-accent-primary/40 flex-shrink-0" />
                  {tocLabels[id]}
                </a>
              ))}
          ...
          <main className="flex-1 min-w-0 space-y-12">
            ...
            <m.div
              ...
              className="rounded-2xl border border-border bg-background-surface p-8 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-5"
            >
              <div>
...
with open("/Users/ubaidadib/Desktop/urm-enroll/src/app/pages/articles/how-to-apply-german-university.tsx", "w", encoding="utf-8") as f:
    f.write(content)
