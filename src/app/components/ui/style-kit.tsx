...
export const slate = {
  ...
  card: "premium-stat-card rounded-2xl transition-all",
  cardSolid: "surface-card rounded-2xl",
      ...
      <div className="absolute inset-0 pointer-events-none">
        {orbs && (
          <>
            ...
            <div className="absolute bottom-0 left-0 w-[800px] h-[800px] bg-accent-primary/7 rounded-full blur-[120px]" />
          </>
        )}
        {grid && (
...
export function Eyebrow({ icon: Icon, children, className = "" }: EyebrowProps) {
  return (
    <m.div
      ...
      viewport={{ once: true }}
      className={`inline-flex items-center gap-3 px-5 py-2.5 premium-eyebrow ${className}`}
    >
...
export function StatCard({ icon: Icon, value, label, note, index = 0, align = "left" }: StatCardProps) {
  ...
  return (
    <m.div
      ...
      className={`p-6 rounded-2xl premium-stat-card transition-all group ${alignClass}`}
    >
      {Icon && (
      ...
      {note && (
        <div className="text-[10px] text-text-muted/75 font-medium bg-bg-secondary border border-border/45 px-2 py-1 rounded-md w-fit mx-auto sm:mx-0">
          {note}
        </div>
      )}
    </m.div>
