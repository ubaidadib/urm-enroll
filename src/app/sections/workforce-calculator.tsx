    ...
    <button
      ...
      className={`px-4 py-2.5 rounded-xl text-sm font-bold border-2 transition-all duration-200 ${
        active
          ...
          : "border-border/70 bg-bg-surface/80 text-text-secondary shadow-sm hover:border-accent-tech/50 hover:text-text-primary"
      }`}
    >
      {children}
    </button>
...
function ResultCard({
  ...
  return (
    <m.div
      ...
      className="relative p-5 sm:p-6 rounded-2xl premium-stat-card overflow-hidden group"
    >
      <div
        className="absolute top-0 right-0 w-20 h-20 rounded-full opacity-10 blur-2xl -translate-y-6 translate-x-6"
        style={{ backgroundColor: accent }}
      />
      <div className="mb-4 flex h-11 w-11 items-center justify-center rounded-xl border border-current/10" style={{ color: accent, backgroundColor: `${accent}14` }}>
        <Icon className="w-5 h-5" />
      </div>
      <div className="text-[10px] font-black uppercase tracking-widest text-text-muted mb-1">
        {label}
      </div>
      <div className="text-2xl sm:text-3xl font-black tabular-nums text-slate-900 dark:text-white leading-tight">
        {value}
      </div>
      <div className="text-xs font-medium text-text-muted mt-1">{sub}</div>
    </m.div>
  );
}
  ...
  return (
    ...
    >
      ...
      <div className="max-w-7xl mx-auto px-[var(--content-gutter)] relative z-10">
        ...
        >
          {/* ── INPUT PANEL ── */}
          <div className="lg:col-span-5 space-y-8 p-8 rounded-3xl surface-card-elevated">
            {/* Language Level */}
            <div>
              <label className="flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-slate-400 mb-4">
