...
import { buildTransporter, getClientIp, hasSmtp, sanitize } from "../email.js";

/**
 * Application submission handler.
 *
 * Accepts the 4-step application form from the program detail page, sends a
 * notification to the admissions team and a confirmation to the applicant.
 * Turnstile and SMTP are both optional — if the env vars are absent (common in
 * local dev) the submission is persisted and acknowledged without sending email.
 */
...
const hasTurnstile = () => Boolean(process.env.TURNSTILE_SECRET_KEY?.trim());
...
export async function handleApplication(request, response) {
  ...
  if (!hasSmtp()) {
    console.info("[api/forms/application] SMTP not configured — application acknowledged without email:", {
      fullName: safeName,
      email: safeEmail,
      programName: safeProgramName,
      universityName: safeUniName,
  ...
  const transporter = buildTransporter("api/forms/application");
  ...
  try {
    ...
    logError(error, request, { endpoint: "api/forms/application" });
