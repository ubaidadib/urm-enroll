 ...
 *
 * Returns the flat programs list that bootstrapMirrorCatalogCache expects from
 * /api/catalog/courses — same shape as toMirrorProgram() in mirror-catalog.ts.
 */
export async function fetchCoursesFromSupabase() {
  ...
  while (true) {
    ...
    if (error) throw new Error(`Supabase courses (flat): ${error.message}`);
    if (!page || page.length === 0) break;
    rows.push(...page);
    if (page.length < PAGE_SIZE) break;
    from += PAGE_SIZE;
  ...
  // Map to the shape mirror-catalog.ts / toMirrorProgram expects
  const data = rows.map((course) => {
    const program = buildProgram(course);
    return {
      // mirror-catalog.ts uses: id, degreeLevel, field, title/name, description,
      // language, duration, tuitionAmount+Currency, countryCode, featured, and
      // the rich structured fields (dates, seasons, country, city, fees, etc.)
      id:               String(course.id),
      universityId:     String(course.university_id ?? ""),
      ...
      duration:         "",
      language:         "",
      tuitionAmount:    program.tuitionPerYear,
      tuitionCurrency:  program.tuitionCurrency,
      description:      program.description,
      countryCode:      course.countries?.name ?? "DE",  // name used as countryCode fallback

      // Rich fields — passed through to toMirrorProgram via payload
      dates:            program.dates,
      seasons:          program.seasons,
      is_seasonal:      program.is_seasonal,
      has_sale:         program.has_sale,
      sale_percentage:  program.sale_percentage,
      fees:             program.fees,
      level:            program.level,
      ...
      toefl_score:      program.toefl_score,
      university:       program.university,
      coverPhoto:       program.coverPhoto,
      feesText:         program.feesText,
      featured:         false,
    };
  });

  return { data, status: "ok", total: data.length };
}
